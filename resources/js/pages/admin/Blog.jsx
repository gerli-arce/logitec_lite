...
export default function AdminBlog() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setShowForm(false)
      setEditingId(null)
      setFormData({ titulo: '', slug: '', contenido: '', fecha_publicacion: new Date().toISOString().split('T')[0] })
    } catch (error) {
    ...
    <AdminLayout>
      <div>
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-bold text-[#1A1A1A]">Blog</h1>
          <button
            onClick={() => {
              setEditingId(null)
              setFormData({ titulo: '', slug: '', contenido: '', fecha_publicacion: new Date().toISOString().split('T')[0] })
              ...
              <tbody>
                {posts.map((post) => (
                  <tr key={post.id} className="border-b hover:bg-gray-50">
                    <td className="px-6 py-4 text-sm text-gray-900">{post.titulo}</td>
